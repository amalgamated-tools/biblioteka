...
2. **Extract metrics with `jq`** into `/tmp/gh-aw/report-site/data.json`:
   ...
   jq -c '{
     ...
     opened_7d:    [.[] | select((.createdAt | fromdateiso8601) > (now - 7*86400))]  | length,
     opened_30d:   [.[] | select((.createdAt | fromdateiso8601) > (now - 30*86400))] | length,
     stale:        [.[] | select(.state=="OPEN") | select((.updatedAt | fromdateiso8601) < (now - 30*86400))] | length,
     ...
     clusters:     [.[] | {
       cluster: (
         ...
         elif (.title | test("feature|feat|enhancement|request";"i")) then "Feature Requests"
     ...
     daily_opened: [.[] | select(.createdAt != null) | .createdAt[:10]] | group_by(.) | map({date:.[0], count:length}) | sort_by(.date) | .[-30:],
...
<meta property="og:image"       content="https://OWNER.github.io/REPO/og-image.svg" />
<meta property="og:type"        content="website" />
<meta property="og:url"         content="https://OWNER.github.io/REPO/" />
<meta name="twitter:card"       content="summary_large_image" />
```

Replace `OWNER`, `REPO`, `YYYY-MM-DD`, and metric values with actual computed values from the data and context. The `og:image` URL must be absolute — construct it from the repository owner and name.
...
Upload this SVG as an asset using the `upload asset` tool (see Phase 6 for details).
...
Use the `upload asset` tool to upload the OGP image. Because the tool only accepts `.png` / `.jpg` / `.jpeg` files, first render `og-image.svg` to PNG:

```bash
# If rsvg-convert is available:
rsvg-convert og-image.svg -w 1200 -h 630 -o og-image.png

# Otherwise, keep the SVG and use the GitHub Pages URL directly
```

If you produce a PNG, upload `/tmp/gh-aw/report-site/og-image.png` as an asset. Collect the returned URL and patch the `og:image` meta tag in `index.html` to use that absolute URL.

If PNG conversion is not available, use the absolute GitHub Pages URL for the SVG (`https://OWNER.github.io/REPO/og-image.svg`) as the `og:image` value — some platforms support SVG OGP images.
