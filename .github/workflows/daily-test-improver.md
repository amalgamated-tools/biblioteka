...
3. Check for existing testing PRs (especially yours with "test: " prefix). Avoid duplicate work.
...
1. List all open PRs with the `test: ` title prefix.
2. For each PR:
...
Maintain a single open issue titled `test: Monthly Activity {YYYY}-{MM}` as a rolling summary of all Test Improver activity for the current month.

1. Search for an open `test: Monthly Activity` issue with label `testing`. If it's for the current month, update it. If for a previous month, close it and create a new one. Read any maintainer comments - they may contain instructions or priorities; note them in memory.
