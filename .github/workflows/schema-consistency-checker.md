...
| `frontend/src/types.ts` or `frontend/src/types/*.ts` or `frontend/src/lib/api.ts` | **LAYER_FRONTEND** |
...
- `frontend/src/types.ts` — Barrel re-export (backward-compat shim)
- `frontend/src/types/*.ts` — Domain-scoped TypeScript interface and type definitions
