...
If **files were changed**, group them by **module/package** (top-level directory or package path) and record which **file types** (languages) are present (e.g., Go, TypeScript, Python). This grouping drives the focused analysis in Phase 2 and avoids loading unnecessary context.

```bash
# Summarize changed files by top-level module and language
git log --since="24 hours ago" --pretty=format:"%H" --no-merges \
  | xargs -r git diff-tree --no-commit-id -r --name-only \
  | grep -v -E '(\.lock$|_test\.|\.gen\.go|vendor/)' \
  | awk -F/ '{print $1}' | sort | uniq -c | sort -rn
```

Record the affected modules list (e.g., `internal/handlers`, `frontend/src/components`) and the language set (e.g., `["go", "typescript"]`). These are used in Phase 2 to limit context loading.
