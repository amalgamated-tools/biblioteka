...
- **Backend**: Go 1.26, standard `net/http` (no router framework), `database/sql`
...
- Make sure to NOT use raw string keys in log fields; use the predefined constants in `internal/otelkeys/logger_keys.go` (package `otelkeys`, e.g., `otelkeys.UserID`) for consistency and to enable better log querying.
- If you need to add a new log field, add a new constant in `internal/otelkeys/logger_keys.go` and use that constant in your logging calls.
