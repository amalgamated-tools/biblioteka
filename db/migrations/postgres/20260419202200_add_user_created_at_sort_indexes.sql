...
-- api_keys: replace single-column user_id index with composite (user_id, created_at DESC, id DESC)
...
-- the temp sort that the single-column idx_api_keys_user_id required.
DROP INDEX IF EXISTS idx_api_keys_user_id;
CREATE INDEX IF NOT EXISTS idx_api_keys_user_created_at ON api_keys (user_id, created_at DESC, id DESC);

-- kobo_tokens: same pattern as api_keys
DROP INDEX IF EXISTS idx_kobo_tokens_user_id;
CREATE INDEX IF NOT EXISTS idx_kobo_tokens_user_created_at ON kobo_tokens (user_id, created_at DESC, id DESC);
...
CREATE INDEX IF NOT EXISTS idx_passkey_credentials_user_created_at ON passkey_credentials (user_id, created_at DESC, id DESC);
