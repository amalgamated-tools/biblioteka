<script lang="ts">
  ...
  import { getConfigStatus, setSmtpConfig, testSmtpConfig } from "../../lib/api";
  import { Send, Mail } from "lucide-svelte";

  interface Props {
    initialConfigured: boolean;
    initialEnvOverride: boolean;
  ...
  let smtpConfigured = $state(initialConfigured);
  let smtpEnvOverride = $state(initialEnvOverride);
  let smtpPasswordSet = $state(initialPasswordSet);
  let smtpHost = $state(initialHost);
  let smtpPort = $state(initialPort);
  let smtpUsername = $state(initialUsername);
  let smtpFrom = $state(initialFrom);
  let smtpTls = $state(initialTls);
  ...
  let saveButtonLabel = $derived.by(() => {
    ...
    if (smtpConfigured) return "Update Configuration";
