...
```
cmd/server/          # Main entry point (server binary)
internal/
...
pkg/
  ...
  plex/              # Generated Plex API client
...
The `*arr` API clients are generated from OpenAPI specs — **never edit `client.gen.go` files by hand**.

```bash
go generate ./...       # Regenerate all clients from local specs
scripts/generate.sh     # Download latest specs + regenerate clients
```
...
- **Handler structure**: Each domain has a handler struct (e.g., `MovieHandler`) holding a `*db.DB` and other dependencies. Handlers live in `internal/handlers/`.
- **JSON responses**: Use `writeJSON(w, status, data)` and `writeError(w, status, message)` from `internal/handlers/helpers.go`.
- **Path parameters**: Extract resource IDs with `extractPathID(path, prefix)` from `internal/handlers/helpers.go` — there is no router with named params. Example: `id, ok := extractPathID(r.URL.Path, "/api/arr-services/")`.
