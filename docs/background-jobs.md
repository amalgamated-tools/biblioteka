...
| `CoverImageURL` (when non-empty) | `books.cover_image_url` | For EPUB files, the cover image is extracted automatically from the archive and stored as a base64 `data:` URL. For other formats (PDF, MOBI, AZW3), cover extraction is not automatic; the field remains `null` unless a URL is set manually via `PUT /api/books/{id}`. |
