...
Each download also records two tracking events (both best-effort — a failure does not block the file transfer):
...
These endpoints require a valid Biblioteka **JWT** (not the OPDS password). They are documented fully in the [API Reference](api/opds.md#opds-credentials).
