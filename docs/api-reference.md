...
Generate a short-lived, single-use nonce to initiate OIDC account linking for the authenticated user. The nonce expires after 5 minutes.
...
Link an OIDC identity to an existing local account. Pass the nonce from [`POST /api/auth/oidc/link-nonce`](#post-apiauthoidclink-nonce) as a query parameter; the browser is then redirected to the OIDC provider to authorize the link.

**Request:** `GET /api/auth/oidc/link?nonce=<nonce>`
...
| Field       | Type    | Required | Description |
|-------------|---------|----------|-------------|
| `file_type` | string  | ✓        | File format (e.g. `"epub"`, `"pdf"`) |
| `file_name` | string  | ✓        | File name |
| `file_path` | string  | ✓        | Absolute path to the file on the server |
| `file_size` | integer |          | File size in bytes |
| `file_hash` | string  |          | Content hash (e.g. `"sha256:abc..."`) |

**Response:** `201 Created` with the new book-file object (same shape as the book-file object above).
