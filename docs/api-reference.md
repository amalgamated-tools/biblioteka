...
Returns `200 OK` with the body `"OK"`. Use this endpoint for liveness probes.
...
Link an OIDC identity to an existing local account. Redirects to the OIDC provider to authorize the link.

---

### `POST /api/auth/oidc/link-nonce` 🔒

Generate a short-lived nonce token that authorises OIDC account linking for the authenticated user. Used internally by the frontend.
