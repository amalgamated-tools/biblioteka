...
**Response body (`200`):** Array of [tag objects](#post-apitags).
...
**Response body (`200`):** Updated array of [tag objects](#post-apitags) — identical to the response from `GET /api/books/{id}/tags`.
...
See [Books](#books) for the book summary object shape. The `total` field reflects the total number of books in the list (not the current page size).
