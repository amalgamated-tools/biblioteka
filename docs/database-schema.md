...
| `goodreads_metadata.go` | `GoodreadsMetadata` struct; `GoodreadsMetadataInput` value-struct (replaces 21 positional parameters with a single named input); `CreateGoodreadsMetadata`, `GetGoodreadsMetadata`, `ListGoodreadsMetadataByUser`, `ListGoodreadsMetadataByStatus`, `UpdateGoodreadsMetadataStatus`, `DeleteGoodreadsMetadata` |
| `find_or_create.go` | Generic `findOrCreate[T]` helper implementing the lookup → insert → race-fetch pattern used by `FindOrCreateAuthor` and `FindOrCreateSeries`; handles concurrent-insert races by catching unique-constraint violations and retrying the fetch |
| `named_entity_write.go` | Generic `namedEntityCreate[T]` / `namedEntityUpdate[T]` helpers that normalize, validate, execute the insert/update, and translate unique-constraint violations into `errExists` sentinel errors; used by all named-entity CRUD functions |
| `scan_helpers.go` | Generic scan utilities: `scanRow[T]` eliminates per-entity scan boilerplate; `collectRows[T]` iterates over `*sql.Rows`; `collectRowsAndTotal[T]` reads a combined-total column alongside each row for paginated queries |
