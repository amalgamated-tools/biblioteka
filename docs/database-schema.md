...
| `goodreads_metadata.go` | `GoodreadsMetadata` struct; `GoodreadsMetadataInput` value-struct for passing optional catalog fields to `CreateGoodreadsMetadata`; `CreateGoodreadsMetadata`, `GetGoodreadsMetadata`, `ListGoodreadsMetadataByUser`, `ListGoodreadsMetadataByStatus`, `UpdateGoodreadsMetadataStatus`, `DeleteGoodreadsMetadata` |
...
| `find_or_create.go` | Generic `findOrCreate[T]` helper implementing the lookup → insert → race-fetch pattern used by `FindOrCreateAuthor` and `FindOrCreateSeries`; handles concurrent-insert races by catching unique-constraint violations and retrying the lookup |
| `named_entity_write.go` | Generic `namedEntityCreate[T]` and `namedEntityUpdate[T]` helpers; normalize name, validate it, execute the insert/update function, and translate unique-constraint violations into sentinel errors — used by all named-entity `Create*` and `Update*` functions |
| `scan_helpers.go` | Generic scan utilities: `scanRow[T]` — creates a value and populates it from a single DB row; `collectRows[T]` — iterates a `*sql.Rows` result set and returns a slice; `collectRowsAndTotal[T]` — same but expects each row to include a `COUNT(*)` total column |
| `tx.go` | `deferRollback` transaction helper — intended for use with `defer`; silently ignores `sql.ErrTxDone` and logs a warning for any other rollback error |
