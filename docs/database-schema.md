...
| `query_helpers.go` | Unexported query utilities: `dollarN(n)` — returns a PostgreSQL-style positional placeholder (`$n`); `buildInClause[T](values, startAt)` — builds positional placeholders and args for SQL `IN` clauses, shared by `books.go`, `book_queries.go`, `book_relations.go`, `book_files.go`, and `kobo_reading_states.go`. |
