...
| Secret | Configured via `JWT_SECRET` environment variable; minimum **required** length is **32 characters** (enforced at startup) |
...
>
> If `JWT_SECRET` is set but shorter than 32 characters, the **server refuses to start** with an error. Either unset `JWT_SECRET` (the server generates a random secret on startup — suitable for development only) or provide a value of at least 32 characters. Use `openssl rand -hex 32` to generate a strong 64-character secret.
