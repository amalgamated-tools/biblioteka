...
func (w *Worker) Start(ctx context.Context) {
	slog.Debug("starting asynq worker", slog.Int("concurrency", DefaultConcurrency))
...
func (w *Worker) Enqueue(ctx context.Context, name string, payload any) (string, error) {
	slog.Debug("enqueuing job", slog.String("job", name))
	...
	slog.Debug("job enqueued", slog.String("job", name), slog.String("task_id", info.ID))
	return info.ID, nil
}
