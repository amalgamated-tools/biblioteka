package db

import (
	"encoding/json"
	"testing"
	"time"
)

func TestTimestamp_Scan_TimeValue(t *testing.T) {
	now := time.Now().UTC()
	var ts Timestamp
	if err := ts.Scan(now); err != nil {
		failNowf(t, "Scan(time.Time) error: %v", err)
	}
	if !ts.Equal(now) {
		failf(t, "got %v, want %v", ts.Time, now)
	}
}

func TestTimestamp_Scan_TimeWithTimezone(t *testing.T) {
	loc := time.FixedZone("EST", -5*60*60)
	est := time.Date(2026, 2, 24, 10, 30, 0, 0, loc)
	var ts Timestamp
	if err := ts.Scan(est); err != nil {
		failNowf(t, "Scan(time.Time with timezone) error: %v", err)
	}
	if !ts.Equal(est) {
		failf(t, "got %v, want %v", ts.Time, est)
	}
}

func TestTimestamp_Scan_Nil(t *testing.T) {
	var ts Timestamp
	ts.Time = time.Now() // set non-zero first
	if err := ts.Scan(nil); err != nil {
		failNowf(t, "Scan(nil) error: %v", err)
	}
	if !ts.IsZero() {
		failf(t, "expected zero time after scanning nil, got %v", ts.Time)
	}
}

func TestTimestamp_Scan_StringFormats(t *testing.T) {
	tests := []struct {
		name  string
		input string
	}{
		{"RFC3339", "2026-02-24T10:30:00Z"},
		{"RFC3339Nano", "2026-02-24T10:30:00.123456789Z"},
		{"RFC3339 with offset", "2026-02-24T10:30:00+05:00"},
		{"datetime with space", "2026-02-24 10:30:00"},
		{"datetime with fractional", "2026-02-24 10:30:00.123456"},
		{"datetime with offset", "2026-02-24 10:30:00.123-05:00"},
		{"datetime T no timezone", "2026-02-24T10:30:00"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var ts Timestamp
			if err := ts.Scan(tt.input); err != nil {
				failNowf(t, "Scan(%q) error: %v", tt.input, err)
			}
			if ts.IsZero() {
				failf(t, "Scan(%q) produced zero time", tt.input)
			}
		})
	}
}

func TestTimestamp_Scan_InvalidString(t *testing.T) {
	var ts Timestamp
	err := ts.Scan("not-a-timestamp")
	if err == nil {
		fail(t, "expected error for invalid string, got nil")
	}
}

func TestTimestamp_Scan_UnsupportedType(t *testing.T) {
	var ts Timestamp
	err := ts.Scan(12345)
	if err == nil {
		fail(t, "expected error for unsupported type, got nil")
	}
}

func TestTimestamp_Scan_StringWithoutTimezoneIsUTC(t *testing.T) {
	var ts Timestamp
	if err := ts.Scan("2026-02-24 10:30:00"); err != nil {
		failNowf(t, "Scan error: %v", err)
	}
	if ts.Location() != time.UTC {
		failf(t, "expected UTC location, got %v", ts.Location())
	}

	var ts2 Timestamp
	if err := ts2.Scan("2026-02-24T10:30:00"); err != nil {
		failNowf(t, "Scan error: %v", err)
	}
	if ts2.Location() != time.UTC {
		failf(t, "expected UTC location, got %v", ts2.Location())
	}
}

func TestTimestamp_MarshalJSON_NonZero(t *testing.T) {
	ts := Timestamp{time.Date(2026, 2, 24, 10, 30, 0, 0, time.UTC)}
	data, err := json.Marshal(ts)
	if err != nil {
		failNowf(t, "MarshalJSON error: %v", err)
	}
	want := `"2026-02-24T10:30:00Z"`
	if string(data) != want {
		failf(t, "got %s, want %s", data, want)
	}
}

func TestTimestamp_MarshalJSON_Zero(t *testing.T) {
	var ts Timestamp
	data, err := json.Marshal(ts)
	if err != nil {
		failNowf(t, "MarshalJSON error: %v", err)
	}
	want := `""`
	if string(data) != want {
		failf(t, "got %s, want %s", data, want)
	}
}

func TestTimestamp_UnmarshalJSON_Valid(t *testing.T) {
	var ts Timestamp
	if err := json.Unmarshal([]byte(`"2026-02-24T10:30:00Z"`), &ts); err != nil {
		failNowf(t, "UnmarshalJSON error: %v", err)
	}
	want := time.Date(2026, 2, 24, 10, 30, 0, 0, time.UTC)
	if !ts.Equal(want) {
		failf(t, "got %v, want %v", ts.Time, want)
	}
}

func TestTimestamp_UnmarshalJSON_Empty(t *testing.T) {
	var ts Timestamp
	ts.Time = time.Now()
	if err := json.Unmarshal([]byte(`""`), &ts); err != nil {
		failNowf(t, "UnmarshalJSON error: %v", err)
	}
	if !ts.IsZero() {
		failf(t, "expected zero time for empty string, got %v", ts.Time)
	}
}

func TestTimestamp_UnmarshalJSON_Null(t *testing.T) {
	var ts Timestamp
	ts.Time = time.Now()
	if err := json.Unmarshal([]byte("null"), &ts); err != nil {
		failNowf(t, "UnmarshalJSON error: %v", err)
	}
	if !ts.IsZero() {
		failf(t, "expected zero time for null, got %v", ts.Time)
	}
}

func TestTimestamp_UnmarshalJSON_Invalid(t *testing.T) {
	var ts Timestamp
	err := json.Unmarshal([]byte(`"not-a-timestamp"`), &ts)
	if err == nil {
		fail(t, "expected error for invalid timestamp, got nil")
	}
}

func TestTimestamp_JSONRoundTrip(t *testing.T) {
	original := Timestamp{time.Date(2026, 6, 15, 14, 0, 0, 0, time.UTC)}
	data, err := json.Marshal(original)
	if err != nil {
		failNowf(t, "Marshal error: %v", err)
	}

	var decoded Timestamp
	if err := json.Unmarshal(data, &decoded); err != nil {
		failNowf(t, "Unmarshal error: %v", err)
	}

	if !original.Equal(decoded.Time) {
		failf(t, "round-trip failed: got %v, want %v", decoded.Time, original.Time)
	}
}
