...
// ScanLibrary enqueues a scan:path job for every path belonging to the given
// library. It returns the number of jobs successfully enqueued.
func ScanLibrary(ctx context.Context, enqueuer Enqueuer, libraryID string, paths []string) int {
	var enqueued int
...
func NewScanLibrariesHandler(lister LibraryLister, enqueuer Enqueuer) func(ctx context.Context, payload []byte) error {
	return func(ctx context.Context, payload []byte) error {
		...
		for _, lib := range libraries {
			...
			enqueued += ScanLibrary(ctx, enqueuer, lib.ID, paths)
		}
