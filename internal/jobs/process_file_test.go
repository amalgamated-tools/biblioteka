...
func TestProcessFileHandler(t *testing.T) {
	...
	if err != nil {
		failNowf(t, "failed to create metadata extractor: %v", err)
	}
	...
	if books[0].Title != "The Great Gatsby" {
		failf(t, "expected title %q, got %q", "The Great Gatsby", books[0].Title)
	}
	...
	if files[0].FileType != "epub" {
		failf(t, "expected file type %q, got %q", "epub", files[0].FileType)
	}
	if files[0].FilePath != epubPath {
		failf(t, "expected file path %q, got %q", epubPath, files[0].FilePath)
	}
	if files[0].FileSize != 1024 {
		failf(t, "expected file size 1024, got %d", files[0].FileSize)
	}
	...
	if len(authors) != 1 {
		failNowf(t, "expected 1 author, got %d", len(authors))
	}
	if authors[0].Name != "F. Scott Fitzgerald" {
		failf(t, "expected author %q, got %q", "F. Scott Fitzgerald", authors[0].Name)
	}

	// Verify ISBN13 was extracted and normalized
	if books[0].ISBN13 == nil || *books[0].ISBN13 != "9780743273565" {
		failf(t, "expected ISBN13 %q, got %v", "9780743273565", books[0].ISBN13)
	}

	// Verify language was extracted from EPUB metadata
	if books[0].Language == nil || *books[0].Language != "en" {
		failf(t, "expected language %q, got %v", "en", books[0].Language)
	}
}
...
func TestProcessFileHandler_MetadataFields(t *testing.T) {
	...
	if book.Title != "Dune" {
		failf(t, "expected title %q, got %q", "Dune", book.Title)
	}
	if book.Description == nil || *book.Description != "A science fiction masterpiece" {
		failf(t, "expected description %q, got %v", "A science fiction masterpiece", book.Description)
	}
	if book.Publisher == nil || *book.Publisher != "Chilton Books" {
		failf(t, "expected publisher %q, got %v", "Chilton Books", book.Publisher)
	}
	if book.PublicationDate == nil || *book.PublicationDate != "1965-08-01" {
		failf(t, "expected publication_date %q, got %v", "1965-08-01", book.PublicationDate)
	}
	if book.Language == nil || *book.Language != "en" {
		failf(t, "expected language %q, got %v", "en", book.Language)
	}
	if book.ISBN13 == nil || *book.ISBN13 != "9780441172719" {
		failf(t, "expected ISBN13 %q, got %v", "9780441172719", book.ISBN13)
	}
	...
	if authors[0].Name != "Frank Herbert" {
		failf(t, "expected author %q, got %q", "Frank Herbert", authors[0].Name)
	}
}
...
func TestProcessFileHandler_AuthorAndLibraryLinking(t *testing.T) {
	...
	if err != nil {
		failNowf(t, "create library: %v", err)
	}
	...
	if err != nil {
		failNowf(t, "list library books: %v", err)
	}
	if len(libBooks) != 1 {
		failNowf(t, "expected 1 library book, got %d", len(libBooks))
	}
}
...
func TestProcessFileHandler_SeriesFromPath(t *testing.T) {
	...
	if err != nil {
		failNowf(t, "get book series: %v", err)
	}
	if len(seriesEntries) != 1 {
		failNowf(t, "expected 1 series entry, got %d", len(seriesEntries))
	}
	if seriesEntries[0].Series.Name != "No. 1 Ladies' Detective Agency" {
		failf(t, "expected series %q, got %q", "No. 1 Ladies' Detective Agency", seriesEntries[0].Series.Name)
	}
	if seriesEntries[0].Position == nil || *seriesEntries[0].Position != 10 {
		failf(t, "expected series position 10, got %v", seriesEntries[0].Position)
	}
	...
	if authors[0].Name != "Alexander McCall Smith" {
		failf(t, "expected author %q, got %q", "Alexander McCall Smith", authors[0].Name)
	}
}
...
func TestProcessFileHandler_DuplicateSkipped(t *testing.T) {
	...
	// Process once.
	if err := handler(context.Background(), payload); err != nil {
		failNowf(t, "first handler call: %v", err)
	}

	// Process again — should be skipped (no error, no duplicate book).
	if err := handler(context.Background(), payload); err != nil {
		failNowf(t, "second handler call: %v", err)
	}
	...
	if len(books) != 1 {
		failf(t, "expected 1 book after duplicate processing, got %d", len(books))
	}
}
